...
class Vars_generator:
    ...
    @staticmethod
    def genVars_nXor_SupP_Blue_isWhite(r):
        return [f'nXor_SupP_Blue_isWhite_r{r}_{j}' for j in range(bs)]

    @staticmethod
    def genVars_nXor_SupP_Red_isWhite(r):
        return [f'nXor_SupP_Red_isWhite_r{r}_{j}' for j in range(bs)]

    @staticmethod
    def genVars_nXor_ConsumedDeg_Blue(r):
        return [f'CD_nXor_Blue_r{r}_{j}' for j in range(bs)]

    @staticmethod
    def genVars_nXor_ConsumedDeg_Red(r):
        return [f'CD_nXor_Red__r{r}_{j}' for j in range(bs)]

    @staticmethod
    def genVars_nXor_SupP_Blue_SumGray(r):
        return [f'G_nXor_SupP_Blue_SumGray_r{r}_{j}' for j in range(bs)]

    @staticmethod
    def genVars_nXor_SupP_Red_SumGray(r):
        return [f'G_nXor_SupP_Red_SumGray_r{r}_{j}' for j in range(bs)]

    @staticmethod
    def genVars_nXor_SupP_Blue_AND(i, r):
        return [f'nXor_SupP_Blue_AND_{i}_r{r}_{j}' for j in range(bs)]

    @staticmethod
    def genVars_nXor_SupP_Red_AND(i, r):
        return [f'nXor_SupP_Red_AND_{i}_r{r}_{j}' for j in range(bs)]
...
class Constraints_generator:
    ...
    def genConstraints_forward_round(self, r):
        ...
        if r != self.mat_r:
            ...
            CD_nXor_Blue = Vars_generator.genVars_nXor_ConsumedDeg_Blue(r)
            SumGray_nXor_SupP_Blue = Vars_generator.genVars_nXor_SupP_Blue_SumGray(r)
            nXor_SupP_Blue_isWhite = Vars_generator.genVars_nXor_SupP_Blue_isWhite(r)
            nXor_SupP_Blue_AND_2 = Vars_generator.genVars_nXor_SupP_Blue_AND(2, r)
            CD_nXor_Red = Vars_generator.genVars_nXor_ConsumedDeg_Red(r)
            SumGray_nXor_SupP_Red = Vars_generator.genVars_nXor_SupP_Red_SumGray(r)
            nXor_SupP_Red_isWhite = Vars_generator.genVars_nXor_SupP_Red_isWhite(r)
            nXor_SupP_Red_AND_1 = Vars_generator.genVars_nXor_SupP_Red_AND(1, r)
            # Xor
            ...
            for bi in range(bs):
                cons = cons + MITMPreConstraints.genConstraints_of_nXor_SupP_Blue_i(
                    Perm_Camellia(bi, IMC_SupP_Blue_1),
                    Perm_Camellia(bi, IMC_SupP_Blue_2),
                    OMC_SupP_Blue_1[bi],
                    OMC_SupP_Blue_2[bi],
                    CD_nXor_Blue[bi],
                    SumGray_nXor_SupP_Blue[bi],
                    nXor_SupP_Blue_isWhite[bi],
                    nXor_SupP_Blue_AND_2[bi]
                )
                cons = cons + MITMPreConstraints.genConstraints_of_nXor_SupP_Red_i(
                    Perm_Camellia(bi, IMC_SupP_Red_1),
                    Perm_Camellia(bi, IMC_SupP_Red_2),
                    OMC_SupP_Red_1[bi],
                    OMC_SupP_Red_2[bi],
                    CD_nXor_Red[bi],
                    SumGray_nXor_SupP_Red[bi],
                    nXor_SupP_Red_isWhite[bi],
                    nXor_SupP_Red_AND_1[bi]
                )
    ...
    def genConstraints_additional(self):
        ...
        for r in range(self.TR):
            if r not in [self.TR - 2, self.TR - 1, 0, 1]:
                CD_Blue = CD_Blue + Vars_generator.genVars_nXor_ConsumedDeg_Blue(r)
                CD_Blue = CD_Blue + Vars_generator.genVars_Xor_ConsumedDeg_Blue(r)
                CD_Red = CD_Red + Vars_generator.genVars_nXor_ConsumedDeg_Red(r)
        ...
        # cons = cons + [BasicTools.plusTerms(CD_Blue) + ' = 5']
        # cons = cons + [BasicTools.plusTerms(CD_Red) + ' = 5']
        # cons = cons + [Deg1 + ' >= 3']
        # cons = cons + [Deg2 + ' >= 2']
        return cons
...
if __name__ == '__main__':
    TR = 15
    root = f'./Model_no_nXor/TR{TR}'
    if not os.path.exists(root):
        os.mkdir(root)
    with open(f"./Model_no_nXor/Result_{TR}.txt", "w") as rd:
        ...
        #     for mat_r in range(4, 5):
                if ini_r != mat_r:
                    filename = f'./Model_no_nXor/TR{TR}/inir{ini_r}_matr{mat_r}'
